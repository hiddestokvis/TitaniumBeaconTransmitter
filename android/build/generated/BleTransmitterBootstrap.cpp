...
#define TAG "com.neverhide.bletransmitter"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> BleTransmitter_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("BleTransmitter.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::BleTransmitterBindings::lookupGeneratedInit(
...
static void BleTransmitter_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(BleTransmitter_getBinding)->GetFunction());
}

static void BleTransmitter_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::BleTransmitterBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry BleTransmitterBinding = {
	"com.neverhide.bletransmitter",
	BleTransmitter_init,
	BleTransmitter_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_neverhide_bletransmitter_BleTransmitterBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.neverhide.bletransmitter", &BleTransmitterBinding);
	titanium::KrollBindings::addExternalLookup(&(::BleTransmitterBindings::lookupGeneratedInit));
}
